...
(59) SchrÃ¶dinger Equation (for ab initio properties): $H\psi = E\psi$
...
graph LR
    subgraph Encoder
        ...
        Enc_Transformer --> Mu[Dense Layer for Î¼]
        Enc_Transformer --> Sigma[Dense Layer for log Ïƒ]
    end
    
    subgraph Latent_Space
        direction TB
        Mu --> Z_Sample[Sample z ~ N(Î¼, Ïƒ)]
...
graph TD
    subgraph Online_Inference
        A[User Prompt] --> B(Scent Generation Policy Ï€_Î¸)
    ...
    subgraph Offline_Training
        ...
        F --> G[Train Reward Model R_Ïˆ]
        G -- Reward Signal --> H
        B -- Initial Policy Ï€_ref --> H
        H(PPO Optimization) --> I[Update Policy Ï€_Î¸]
    end
...
graph TD
    ...
    A & C --> E(Integral over Time: âˆ« WellBeing * EnvHealth dt)
