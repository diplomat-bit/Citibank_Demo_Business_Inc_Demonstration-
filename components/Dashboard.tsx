    ...
    return (
        <Card title="Anomaly Alerts" variant="interactive" onClick={onClick} className={count > 0 ? 'border-yellow-500/80 shadow-yellow-500/10' : ''}>
            <div className="flex flex-col justify-center items-center h-full text-center">
                ...
                <div className={`mt-4 px-4 py-2 ${count > 0 ? 'bg-yellow-600/50' : 'bg-cyan-600/50'} text-white rounded-lg text-sm font-medium`}>
                    Investigate
                </div>
            </div>
        </Card>
...
const MonthlyCarbonSummary: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
    const totalFootprint = transactions.reduce((sum, tx) => sum + (tx.carbonFootprint || 0), 0);
    return (
        <Card title="Monthly Carbon Summary">
            <div className="text-center">
                <p className="text-3xl font-bold text-cyan-300">{totalFootprint.toFixed(1)} kg CO₂</p>
                <p className="text-sm text-gray-400">Estimated total for this month's transactions.</p>
                <p className="text-xs text-gray-500 mt-2">AI Tip: Dining out has a higher footprint than groceries.</p>
            </div>
        </Card>
    );
};
...
const Dashboard: React.FC<DashboardProps> = ({ setActiveView }) => {
    ...
    const { transactions, impactData, gamification, subscriptions, creditScore, upcomingBills, savingsGoals, marketMovers, budgets, linkedAccounts, rewardPoints, isImportingData, financialAnomalies } = context;
        ...
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-3">
                 ...
                 <div className="lg:col-span-3">
                    <AnomalyAlertWidget count={newAnomaliesCount} onClick={() => setActiveView(View.Anomalies)} />
                </div>
