...
const ApiKeysView: React.FC = () => {
    ...
    if (!context) throw new Error("ApiKeysView must be within a DataProvider");

    const { apiKey, generateApiKey, isLoading } = context;
    const [newKey, setNewKey] = useState<string | null>(null);
    const [isGenerating, setIsGenerating] = useState(false);
    const [copySuccess, setCopySuccess] = useState('');
    const [isAiModalOpen, setAiModalOpen] = useState(false);
    const [aiFeature, setAiFeature] = useState<'audit' | 'scope' | 'codegen' | 'leak' | null>(null);
        ...
        try {
            ...
            switch(type) {
                ...
                case 'leak':
                     ...
                     // Special handling for a more realistic simulated response
        ...
        <>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <Card className="text-center"><p className="text-3xl font-bold text-white">3</p><p className="text-sm text-gray-400 mt-1">Active Keys</p></Card>
                ...
                <Card>
                    ...
                    <div className="mt-4 border-t border-gray-700 pt-4">{apiKey ? (<div className="flex justify-between items-center p-3 bg-gray-800/50 rounded-lg"><div><p className="font-mono text-white">{apiKey.substring(0, 9)}... <span className="text-gray-400">(Live Key)</span></p><p className="text-xs text-gray-500">Created on: {new Date().toLocaleDateString()}</p></div><button className="text-sm text-red-400 hover:underline">Revoke</button></div>) : (<p className="text-gray-500 text-center py-4">You don't have any API keys yet.</p>)}</div>
                </Card>
            </div>
