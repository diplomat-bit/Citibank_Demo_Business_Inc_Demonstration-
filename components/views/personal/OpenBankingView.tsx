...
import React, { useState, useReducer, useEffect, useCallback, useMemo, FC, useRef } from 'react';
 ...
 */
export type ConnectionStatus = 'active' | 'expired' | 'revoked' | 'pending' | 'reauthentication_required';
...
export interface BankAccount {
    ...
    name: string;
    institution: string;
    type: 'checking' | 'savings' | 'credit_card' | 'loan' | 'investment' | 'mortgage';
...
/**
 * Detailed information about a specific permission (scope).
 */
export interface PermissionDetail {
    key: string;
    label: string;
    description: string;
    category: 'Account Information' | 'Payment Initiation' | 'Data Analysis' | 'Profile Information';
...
export interface ThirdPartyApp {
    ...
    developer: string;
    website: string;
    logoUrl: string; // URL to a high-res logo
    privacyPolicyUrl: string;
    termsOfServiceUrl: string;
    category: 'Budgeting' | 'Tax' | 'Investment' | 'Lending' | 'Analytics' | 'Wealth Management';
    icon: string; // SVG path for simple icons
...
export interface OpenBankingConnection {
    ...
    lastConsentReauthenticatedAt: string | null;
...
/**
 * Represents an event in the connection's history log.
 */
export interface ConnectionHistoryEvent {
    id: string;
    connectionId: number;
    appName: string;
    eventType: 'connected' | 'revoked' | 'expired' | 'data_accessed' | 'permissions_updated' | 'reauthenticated' | 'failed_access_attempt';
...
export interface OpenBankingSettings {
    ...
    notifyOnDataAccessed: boolean;
    requireReauthenticationForHighRisk: boolean;
}

/**
 * Represents an AI-driven security insight.
 */
export interface AISecurityInsight {
    ...
    relatedConnectionId?: number;
}

/**
 * Represents an AI-powered app recommendation.
 */
export interface AIAppRecommendation {
    id: string;
    app: ThirdPartyApp;
    reason: string;
...
const MOCK_ACCOUNTS: BankAccount[] = [
    { id: 'acc_101', name: 'Primary Checking', institution: 'First National Bank', type: 'checking', accountNumberMasked: '**** **** **** 1234', balance: 15432.88, currency: 'USD' },
    { id: 'acc_102', name: 'High-Yield Savings', institution: 'First National Bank', type: 'savings', accountNumberMasked: '**** **** **** 5678', balance: 89102.15, currency: 'USD' },
    { id: 'acc_103', name: 'Travel Rewards Card', institution: 'Capital One', type: 'credit_card', accountNumberMasked: '**** ****** *9012', balance: -2345.67, currency: 'USD' },
    { id: 'acc_104', name: 'Brokerage Account', institution: 'Fidelity', type: 'investment', accountNumberMasked: '****-**-7789', balance: 250341.76, currency: 'USD' },
    { id: 'acc_105', name: 'Home Mortgage', institution: 'First National Bank', type: 'mortgage', accountNumberMasked: '****-****-1005', balance: -285000.00, currency: 'USD' },
];
...
export const ALL_PERMISSIONS: { [key: string]: PermissionDetail } = {
    ...
    'access_contact_info': { key: 'access_contact_info', label: 'Access contact information', description: 'Allows the app to view your name, address, and email associated with your bank account.', category: 'Profile Information', riskLevel: 'low' },
    'manage_investments': { key: 'manage_investments', label: 'Manage investments', description: 'Allows the app to buy or sell securities on your behalf. This is a high-risk permission.', category: 'Payment Initiation', riskLevel: 'high' },
};

export const MOCK_AVAILABLE_APPS: ThirdPartyApp[] = [
    { id: 'app_001', name: 'MintFusion Budgeting', developer: 'FusionCorp', website: 'https://fusioncorp.example.com', logoUrl: '/logos/mintfusion.svg', privacyPolicyUrl: '#', termsOfServiceUrl: '#', description: 'A powerful tool to track your spending, create budgets, and achieve your financial goals.', category: 'Budgeting', icon: 'M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z', requestedPermissions: [ALL_PERMISSIONS.read_transaction_history, ALL_PERMISSIONS.view_account_balances, ALL_PERMISSIONS.view_account_details] },
    { id: 'app_002', name: 'TaxBot Pro', developer: 'Taxable Inc.', website: 'https://taxable.example.com', logoUrl: '/logos/taxbot.svg', privacyPolicyUrl: '#', termsOfServiceUrl: '#', description: 'Automate your tax preparation by importing financial data directly and securely.', category: 'Tax', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', requestedPermissions: [ALL_PERMISSIONS.read_transaction_history, ALL_PERMISSIONS.access_income_statements] },
    { id: 'app_003', name: 'Acornvest', developer: 'Oak Financial', website: 'https://oakfin.example.com', logoUrl: '/logos/acornvest.svg', privacyPolicyUrl: '#', termsOfServiceUrl: '#', description: 'Invest your spare change automatically from everyday purchases.', category: 'Investment', icon: 'M13 17h8m0 0V9m0 8l-8-8-4 4-6-6', requestedPermissions: [ALL_PERMISSIONS.read_transaction_history, ALL_PERMISSIONS.view_account_balances, ALL_PERMISSIONS.initiate_single_payment] },
    { id: 'app_004', name: 'LendEasy', developer: 'QuickCredit', website: 'https://quickcredit.example.com', logoUrl: '/logos/lendeasy.svg', privacyPolicyUrl: '#', termsOfServiceUrl: '#', description: 'Get faster loan approvals by securely sharing your financial history.', category: 'Lending', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z', requestedPermissions: [ALL_PERMISSIONS.read_transaction_history, ALL_PERMISSIONS.view_account_balances, ALL_PERMISSIONS.access_income_statements, ALL_PERMISSIONS.access_contact_info] },
    { id: 'app_005', name: 'WealthFrontier', developer: 'Apex Wealth', website: 'https://apexwealth.example.com', logoUrl: '/logos/wealthfrontier.svg', privacyPolicyUrl: '#', termsOfServiceUrl: '#', description: 'A comprehensive wealth management platform to track and grow your portfolio.', category: 'Wealth Management', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', requestedPermissions: [ALL_PERMISSIONS.view_account_balances, ALL_PERMISSIONS.read_transaction_history, ALL_PERMISSIONS.manage_investments] },
];

const MOCK_CONNECTIONS: OpenBankingConnection[] = [
    { id: 1, app: MOCK_AVAILABLE_APPS[0], status: 'active', connectedAt: '2023-08-15T10:30:00Z', expiresAt: '2024-11-15T10:30:00Z', lastAccessedAt: '2023-10-28T14:00:00Z', lastConsentReauthenticatedAt: '2023-08-15T10:30:00Z', linkedAccountIds: ['acc_101', 'acc_102'], grantedPermissions: MOCK_AVAILABLE_APPS[0].requestedPermissions, dataSharingFrequency: 'recurring' },
    { id: 2, app: MOCK_AVAILABLE_APPS[1], status: 'active', connectedAt: '2023-01-20T18:00:00Z', expiresAt: '2024-04-20T18:00:00Z', lastAccessedAt: '2023-04-15T09:00:00Z', lastConsentReauthenticatedAt: '2023-01-20T18:00:00Z', linkedAccountIds: ['acc_101'], grantedPermissions: MOCK_AVAILABLE_APPS[1].requestedPermissions, dataSharingFrequency: 'recurring' },
    { id: 3, app: MOCK_AVAILABLE_APPS[3], status: 'expired', connectedAt: '2022-05-10T11:00:00Z', expiresAt: '2023-08-10T11:00:00Z', lastAccessedAt: '2022-05-10T11:05:00Z', lastConsentReauthenticatedAt: '2022-05-10T11:00:00Z', linkedAccountIds: ['acc_101', 'acc_102'], grantedPermissions: MOCK_AVAILABLE_APPS[3].requestedPermissions, dataSharingFrequency: 'one_time' },
    { id: 4, app: MOCK_AVAILABLE_APPS[4], status: 'reauthentication_required', connectedAt: '2024-01-05T12:00:00Z', expiresAt: '2024-04-05T12:00:00Z', lastAccessedAt: '2024-02-15T10:00:00Z', lastConsentReauthenticatedAt: '2024-01-05T12:00:00Z', linkedAccountIds: ['acc_104'], grantedPermissions: MOCK_AVAILABLE_APPS[4].requestedPermissions, dataSharingFrequency: 'recurring' },
];

const MOCK_HISTORY: ConnectionHistoryEvent[] = [
    { id: 'evt_001', connectionId: 1, appName: 'MintFusion Budgeting', eventType: 'connected', timestamp: '2023-08-15T10:30:00Z', details: 'Access granted to Primary Checking, High-Yield Savings.', ipAddress: '72.229.28.185' },
    ...
    { id: 'evt_003', connectionId: 2, appName: 'TaxBot Pro', eventType: 'connected', timestamp: '2023-01-20T18:00:00Z', details: 'Access granted to Primary Checking.', ipAddress: '107.178.194.30' },
    ...
    { id: 'evt_005', connectionId: 3, appName: 'LendEasy', eventType: 'connected', timestamp: '2022-05-10T11:00:00Z', details: 'Access granted for a one-time credit check.', ipAddress: '192.0.2.146' },
    ...
    { id: 'evt_008', connectionId: 4, appName: 'WealthFrontier', eventType: 'connected', timestamp: '2024-01-05T12:00:00Z', details: 'Access granted to Brokerage Account.', ipAddress: '66.249.66.1' },
    { id: 'evt_009', connectionId: 4, appName: 'WealthFrontier', eventType: 'failed_access_attempt', timestamp: '2024-03-01T09:00:00Z', details: 'App attempted to access data, but consent requires reauthentication.' },
];
...
const MOCK_USER_SETTINGS: OpenBankingSettings = {
    ...
    notifyOnDataAccessed: false,
    requireReauthenticationForHighRisk: true,
};

const MOCK_AI_SECURITY_INSIGHTS: AISecurityInsight[] = [
    { id: 'ai_01', title: 'High-Risk Permission Active', description: 'The "WealthFrontier" app has permission to manage investments, which is a high-risk activity.', severity: 'warning', recommendation: 'Review this connection regularly. If you are not actively using its investment features, consider revoking access.', relatedConnectionId: 4},
    { id: 'ai_02', title: 'Dormant Connection', description: 'The "TaxBot Pro" app has not accessed your data in over 6 months.', severity: 'info', recommendation: 'If you have completed your taxes for the year, you can safely revoke access until you need it again next year to minimize data exposure.', relatedConnectionId: 2},
    { id: 'ai_03', title: 'Multiple Budgeting Apps', description: 'You have connected "MintFusion Budgeting". Our systems show you previously connected another budgeting app.', severity: 'info', recommendation: 'Consolidating to a single app can simplify your financial overview and reduce the number of parties with access to your data.'}
];
...
export const StatusTag: FC<{ status: ConnectionStatus }> = ({ status }) => {
    const statusStyles = {
        ...
        reauthentication_required: 'bg-orange-500/20 text-orange-300',
    };
    return (
...
type Action =
    | { type: 'FETCH_START' }
    | { type: 'FETCH_SUCCESS'; payload: { connections: OpenBankingConnection[], history: ConnectionHistoryEvent[], settings: OpenBankingSettings, availableApps: ThirdPartyApp[], accounts: BankAccount[], aiSecurityInsights: AISecurityInsight[] } }
    ...
    switch (action.type) {
        ...
        case 'ADD_CONNECTION': {
            ...
            const newConnection: OpenBankingConnection = {
                ...
                lastAccessedAt: null,
                lastConsentReauthenticatedAt: now.toISOString(),
                linkedAccountIds,
                grantedPermissions,
        ...
        <Modal isOpen={!!connection} onClose={onClose} title={`${connection.app.name} Details`}>
            <div className="space-y-6 text-gray-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="bg-gray-700/50 p-3 rounded-lg"><p className="text-gray-400 text-xs">Status</p><div className="mt-1"><StatusTag status={connection.status} /></div></div>
                    <div className="bg-gray-700/50 p-3 rounded-lg"><p className="text-gray-400 text-xs">Connected On</p><p className="text-white font-medium">{formatDate(connection.connectedAt)}</p></div>
                    <div className="bg-gray-700/50 p-3 rounded-lg"><p className="text-gray-400 text-xs">Expires On</p><p className="text-white font-medium">{formatDate(connection.expiresAt)}</p></div>
                </div>
                <div>
...
export const ConnectNewAppWizard: FC<{ isOpen: boolean; onClose: () => void; availableApps: ThirdPartyApp[]; accounts: BankAccount[]; onConnect: (payload: { app: ThirdPartyApp; linkedAccountIds: string[]; grantedPermissions: PermissionDetail[] }) => void; }> = ({ isOpen, onClose, availableApps, accounts, onConnect }) => {
    ...
    const handleConfirm = () => {
        ...
        onConnect({ app: selectedApp, linkedAccountIds: selectedAccountIds, grantedPermissions: selectedApp.requestedPermissions });
        setStep(4);
    };
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {availableApps.map(app => (
                                <div key={app.id} onClick={() => handleSelectApp(app)} className="p-4 bg-gray-700/50 rounded-lg flex items-start gap-4 cursor-pointer hover:bg-gray-700 transition-colors">
                                    ...
                                    <div><p className="font-semibold text-white">{app.name}</p><p className="text-xs text-gray-400">{app.description}</p></div>
                                </div>
                            ))}
                        </div>
                    </div>
                 ...
                 return (
                    <div>
                        ...
                        <div className="space-y-3">
                         ...
                         <div className="mt-6 flex justify-end"><button onClick={handleConfirm} disabled={selectedAccountIds.length === 0} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg disabled:bg-gray-600 disabled:cursor-not-allowed">Confirm and Connect</button></div>
                    </div>
                 );
            case 4:
                return (
...
const AISecurityAdvisor: FC<{ insights: AISecurityInsight[] }> = ({ insights }) => {
    ...
    return (
        <Card title="AI Security Advisor" titleTooltip="Our AI analyzes your connections for potential risks and offers recommendations to enhance your security.">
            <div className="space-y-4">
                {insights.map(insight => (
                    <div key={insight.id} className={`p-4 border-l-4 rounded-r-lg ${severityStyles[insight.severity]}`}>
                        <h4 className="font-bold text-white">{insight.title}</h4>
                        <p className="text-sm text-gray-300 mt-1">{insight.description}</p>
                        <p className="text-sm font-medium text-gray-200 mt-2 pt-2 border-t border-white/10">Recommendation: <span className="font-normal">{insight.recommendation}</span></p>
                    </div>
                ))}
            </div>
        </Card>
...
const OpenBankingView: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [connections, history, settings, availableApps, accounts, aiSecurityInsights] = await Promise.all([
                    ...
                    mockApi(MOCK_AI_SECURITY_INSIGHTS, 1200),
                ]);
                dispatch({ type: 'FETCH_SUCCESS', payload: { connections, history, settings, availableApps, accounts, aiSecurityInsights } });
            } catch (error) {
    ...
    const filteredConnections = useMemo(() => {
        return state.connections.filter(c => {
            const matchesSearch = c.app.name.toLowerCase().includes(debouncedFilter.toLowerCase());
            const matchesStatus = state.statusFilter === 'all' || c.status === state.statusFilter;
            return matchesSearch && matchesStatus;
        });
    }, [state.connections, debouncedFilter, state.statusFilter]);
    ...
    return (
        <div className="space-y-8">
            ...
            <AISecurityAdvisor insights={state.aiSecurityInsights} />
            ...
            <Card title="Your Connected Applications" titleTooltip="This is a list of all third-party applications you have granted access to your financial data.">
                <div className="mb-4 flex flex-col sm:flex-row gap-4">
                    ...
                    >
                        ...
                        <option value="reauthentication_required">Reauthentication Required</option>
                    </select>
                </div>
                    ...
                    {!state.isLoading && !state.error && filteredConnections.map(conn => (
                        <div key={conn.id} className="p-4 bg-gray-800/50 rounded-lg flex flex-col sm:flex-row justify-between items-start gap-4">
                            ...
                            <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2 items-stretch sm:items-center flex-shrink-0 self-start sm:self-center">
                                ...
                                {conn.status === 'reauthentication_required' && (
                                    <button className="px-3 py-2 bg-orange-500/50 hover:bg-orange-600 text-white rounded-lg text-xs w-full sm:w-auto">Re-authenticate</button>
                                )}
                            </div>
                        </div>
                    ))}
                ...
                <div className="text-sm text-gray-300 space-y-3 max-h-96 overflow-y-auto pr-2">
                    {state.history.map(event => (
                        <div key={event.id} className="flex gap-4 items-start">
                            <div className="text-xs text-gray-500 whitespace-nowrap pt-1">{formatDate(event.timestamp)}</div>
