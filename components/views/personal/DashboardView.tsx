...
import type { GamificationState, Subscription, CreditScore, SavingsGoal, MarketMover, UpcomingBill, Transaction, BudgetCategory, RewardPoints } from '../../../types';
...
const WIDGET_ICONS: { [key: string]: React.FC<{ className?: string }> } = {
    ...
    anomaly: ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 20.944a11.955 11.955 0 019-2.606m0-15.394v15.394" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 12h2l2-4 2 4h2" /></svg>,
};
...
const LinkAccountPrompt: React.FC = () => {
    ...
    return (
        <Card title="Welcome to Demo Bank">
...
const RewardPointsWidget: React.FC<{ rewards: RewardPoints; onClick: () => void; }> = ({ rewards, onClick }) => (
    <Card title="Rewards Points" className="h-full" variant="interactive" onClick={onClick}>
        <div className="flex flex-col justify-center items-center h-full text-center">
            ...
            <div className="mt-4 px-4 py-2 bg-cyan-600/50 text-white rounded-lg text-sm font-medium">View Rewards</div>
        </div>
    </Card>
);
...
const AnomalyAlertWidget: React.FC<{ count: number; onClick: () => void; }> = ({ count, onClick }) => {
    ...
    return (
        <Card title="Anomaly Alerts" variant="interactive" onClick={onClick} className={count > 0 ? 'border-yellow-500/80 shadow-yellow-500/10' : ''}>
            <div className="flex flex-col justify-center items-center h-full text-center">
                <Icon className={`h-10 w-10 ${count > 0 ? 'text-yellow-400' : 'text-green-400'}`} />
                <p className="text-4xl font-bold text-white mt-2">{count}</p>
                <p className="text-gray-400 text-sm">New Anomalies Detected</p>
                <div className={`mt-4 px-4 py-2 ${count > 0 ? 'bg-yellow-600/50' : 'bg-cyan-600/50'} text-white rounded-lg text-sm font-medium`}>Investigate</div>
            </div>
        </Card>
    );
};
            ...
            {subscriptions.map(sub => {
                ...
                return (
                    <div key={sub.id} className="flex items-center justify-between text-sm">
                        <div className="flex items-center"><Icon className="w-5 h-5 text-cyan-400 mr-3" /><span className="text-gray-200">{sub.name}</span></div>
        ...
        <div className="space-y-3">
            {bills.map(bill => (
                <div key={bill.id} className="flex items-center justify-between text-sm p-2 rounded-lg hover:bg-gray-700/50">
                    <div><p className="text-gray-200">{bill.name}</p><p className="text-xs text-gray-400">{bill.dueDate}</p></div>
                    <div className="text-right"><p className="font-mono text-white">${bill.amount.toFixed(2)}</p></div>
...
const CategorySpending: React.FC<{ budgets: BudgetCategory[]; onClick: () => void; }> = ({ budgets, onClick }) => {
    ...
    return (
        <Card title="Spending by Category" variant="interactive" onClick={onClick}>
            <div className="h-48"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} cx="50%" cy="50%" innerRadius={40} outerRadius={60} dataKey="value" paddingAngle={5}>{data.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip contentStyle={{ backgroundColor: 'rgba(31, 41, 55, 0.8)', borderColor: '#4b5563' }} /><Legend iconSize={8} wrapperStyle={{fontSize: '12px'}} /></PieChart></ResponsiveContainer></div>
        </Card>
    );
};
...
const CashFlowAnalysis: React.FC<{ transactions: Transaction[]; onClick: () => void; }> = ({ transactions, onClick }) => {
    ...
    return (
        <Card title="Cash Flow" variant="interactive" onClick={onClick}>
            <div className="h-48"><ResponsiveContainer width="100%" height="100%"><BarChart data={monthlyFlows} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}><XAxis dataKey="name" stroke="#9ca3af" fontSize={12} /><YAxis stroke="#9ca3af" fontSize={12} /><Tooltip contentStyle={{ backgroundColor: 'rgba(31, 41, 55, 0.8)', borderColor: '#4b5563' }} /><Legend wrapperStyle={{fontSize: '12px'}} /><Bar dataKey="income" fill="#10b981" name="Income" /><Bar dataKey="expense" fill="#f43f5e" name="Expense" /></BarChart></ResponsiveContainer></div>
        </Card>
    );
};
    ...
    <Card title="Savings Goals" className="h-full" variant="interactive" onClick={onClick}>
        <div className="space-y-4">
            {goals.map(goal => {
                ...
                return (
                    <div key={goal.id}>
                        <div className="flex justify-between items-center mb-1"><div className="flex items-center"><Icon className="w-5 h-5 text-cyan-400 mr-2" /><span className="text-sm font-medium text-white">{goal.name}</span></div><span className="text-xs font-mono text-gray-300">{progress}%</span></div>
        ...
        <div className="space-y-1">
            {movers.map(mover => (
                <div key={mover.ticker} onClick={() => onSelect(mover)} className="flex items-center justify-between text-sm p-2 rounded-lg cursor-pointer hover:bg-gray-700/50">
                    <div><p className="font-bold text-white">{mover.ticker}</p><p className="text-xs text-gray-400 truncate w-32">{mover.name}</p></div>
                    <div className="text-right"><p className="font-mono text-white">${mover.price.toFixed(2)}</p><p className={`text-xs ${mover.change > 0 ? 'text-green-400' : 'text-red-400'}`}>{mover.change > 0 ? '+' : ''}{mover.change.toFixed(2)}</p></div>
                </div>
            ))}
        </div>
    </Card>
...
const DashboardView: React.FC<DashboardProps> = ({ setActiveView }) => {
    ...
    const { transactions, gamification, subscriptions, creditScore, upcomingBills, savingsGoals, marketMovers, budgets, linkedAccounts, rewardPoints, isImportingData, financialAnomalies, impactData } = context;
    ...
    const handleQuickAction = (action: string) => {
        if (action === 'Send Money') setActiveView(View.SendMoney); else setModal({ type: action, data: null });
    };

    const mockStockData = useMemo(() => Array.from({ length: 30 }, (_, i) => ({
        day: i, price: modal?.data?.price ? modal.data.price - 15 + Math.random() * 30 : 100 + Math.random() * 50
    })), [modal?.data?.price]);
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-6"><MarketMovers movers={marketMovers} onSelect={(mover) => setModal({ type: 'StockDetail', data: mover })} /></div>
                <div className="lg:col-span-6"><UpcomingBills bills={upcomingBills} onPay={(bill) => setModal({ type: 'Pay Bill', data: bill })} /></div>
            ...
            <Modal isOpen={modal?.type === 'Deposit'} onClose={() => setModal(null)} title="Deposit Check"><p>Mobile check deposit functionality would be implemented here, likely using the device camera.</p></Modal>
