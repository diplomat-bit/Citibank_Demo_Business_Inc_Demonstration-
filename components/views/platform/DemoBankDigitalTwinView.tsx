...
const DemoBankDigitalTwinView: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const fullPrompt = `Generate a simple JSON schema for a digital twin model based on this description: "${prompt}".`;
    ...
    return (
        <div className="space-y-6">
            ...
            <Card title="AI Twin Model Generator">
                <p className="text-gray-400 mb-4">Describe the physical asset you want to model.</p>
                <textarea
                    value={prompt}
                ...
                <button onClick={handleGenerate} disabled={isLoading} className="w-full mt-2 py-2 bg-cyan-600 hover:bg-cyan-700 rounded disabled:opacity-50">
                    {isLoading ? 'Generating...' : 'Generate Twin Schema'}
                </button>
            </Card>

            {generatedSchema && (
                <Card title="Generated Twin Schema">
                    <button onClick={() => setGeneratedSchema(null)} className="absolute top-4 right-4 text-gray-400 hover:text-white">&times;</button>
