            ...
            <Card className="flex-grow flex flex-col" padding="none">
                ...
                {messages.length === 0 && (
                    <div className="text-center p-6 text-gray-400">
                        <p className="mb-4">Ask me anything about your finances. For example:</p>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                             {prompts.map(p => <button key={p} onClick={() => handleSendMessage(p)} className="p-2 bg-gray-700/50 hover:bg-gray-700 rounded-lg text-sm text-cyan-200 transition-colors">"{p}"</button>)}
                        </div>
                    </div>
                )}
                <div className="p-4 border-t border-gray-700/60 bg-gray-800/50 rounded-b-xl">
                    <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(input); }} className="flex items-center gap-2">
                        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Quantum anything..." className="flex-grow bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2 text-white" disabled={isLoading} />
                        <button type="submit" className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg disabled:opacity-50" disabled={isLoading}>{isLoading ? '...' : 'Send'}</button>
                    </form>
                </div>
            </Card>
        </div>
