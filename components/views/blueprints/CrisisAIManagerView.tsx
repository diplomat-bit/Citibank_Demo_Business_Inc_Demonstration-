          ...
          {sortedAgentLogs.map(log => {
            ...
            return (
              <div key={log.id} className="bg-gray-800 p-4 rounded-lg shadow">
                ...
                {/* Modified this section based on the error output snippet for a more detailed display */}
                <div className="text-white text-md mb-2">
                  <strong>[{new Date(log.timestamp).toLocaleTimeString()}] {agent?.name || log.agentId}:</strong> {log.action} -
                  <pre style={{ display: 'inline', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                    {JSON.stringify(log.details).substring(0, 100)}{log.details.length > 100 ? '...' : ''}
                  </pre>
                </div>
          ...
          {Object.values(CrisisType).map(type => (
            <div key={type} className="mb-4">
              ...
              <textarea
                ...
                onChange={e => updateSetting('defaultTemplates', { ...settings.defaultTemplates[type], pressRelease: e.target.value } as CommsPackage)}
