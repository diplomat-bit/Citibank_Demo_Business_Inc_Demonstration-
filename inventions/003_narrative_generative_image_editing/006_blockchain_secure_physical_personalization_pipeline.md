...
```mermaid
sequenceDiagram
    participant User as User Client (Multi-Dimensional DID & Bio-Haptic, Quantum Wallet)
    participant API as API Gateway (Quantum-Encrypted, Load-Balanced)
    participant Orchestrator as Orchestration Service (O'Callaghan Quantum Core & Perpetual Homeostasis Engine)
    participant BlockchainService as Blockchain Interaction Service (Quantum-Hardened, Lattice-Crypto)
    participant Blockchain as Fractal-Lattice Blockchain Layer (Self-Healing)
    participant IPFS as IPFS/Distributed Storage (Content-Addressable Quantum Storage, Self-Healing)
    participant KMS as Key Management Service (Distributed, Self-Healing, QKD)
    participant Oracle as Quantum Oracle Service (Pre-Cognitive, Ethical Data Sourcing)

    User->>User: 1. Receive Physical Card (Self-Aware Artifact with Quantum-Entangled UPI)
    User->>User: 2. Launch Verification Feature, Scan Quantum-Entangled UPI (NFC/QR/Bio-Integrated, Molecular Imprint)
    User->>User: 2a. Perform Quantum Challenge-Response (Local Quantum Coherence Check)
    User->>API: 3. Send UPI & User DID for Verification (incl. ZKP for privacy, Post-Quantum Entanglement Verifiable)
    API->>Orchestrator: 4. Forward Verification Request (Quantum-Secure Channel, Ethical Context Verified)
    Orchestrator->>BlockchainService: 5. Query Digital Twin by UPI and verify ZKP (Multidimensional Hash Array, Ethical Lineage)
    BlockchainService->>Blockchain: 6. Request Digital Twin Data (DTID, Metadata Hash Array, Image Ref, Ethical Provenance) & Spatiotemporal Transaction History (Production, Supply Chain, Environmental, Ethical Audits)
    Blockchain-->>BlockchainService: 7. Return DT Record & History (Fractal-Lattice Retrieval, Quantum-Sealed)
    BlockchainService-->>Orchestrator: 8. Return DT Record & History (with Predictive Integrity Score)
    Orchestrator->>KMS: 9. Request Decryption Key (if needed for sensitive metadata, Quantum-Derived via QKD)
    KMS-->>Orchestrator: 10. Return Decryption Key (with Temporal Validity)
    Orchestrator->>IPFS: 11. Retrieve Associated High-Res Image using Image Ref (e.g., IPFS hash, from Content-Addressable Quantum Storage, Self-Healing)
    IPFS-->>Orchestrator: 12. Return High-Res Image Data (Verifiably Pristine)
    Orchestrator-->>API: 13. Return Digital Twin Data, Decrypted Metadata, Image, & Full Provenance History (Multiversal, with Ethical & Environmental Score)
    API-->>User: 14. Display Authenticity Verification Details (incl. AR-PAD overlay, Predictive Anomaly Detection, Ethical Lineage Visualizer)
    User->>User: 15. Visually Compare Physical Card with Digital Twin Record & History (Bio-Haptic Confirmation, Quantum Coherence Feedback)
    User->>User: 16. Confirm Authenticity or Initiate Dispute (via AI-Driven Ethical Dispute Resolution SC, Homeostasis Engine)
    alt If Discrepancy Detected (or Predicted)
        User->>Orchestrator: 17. Initiate Dispute with Probabilistic Proof (e.g., photo, detailed description, bio-haptic anomaly report, ethical concern statement)
        Orchestrator->>BlockchainService: 18. Trigger AI-Driven Ethical Dispute Resolution Smart Contract (Perpetual Homeostasis Engine)
        BlockchainService->>Blockchain: 19. Record Dispute & Evidence (Multiversal, Quantum-Sealed)
        Blockchain->>Oracle: 20. Request external evidence (e.g., atmospheric particulate analysis during transit, quantum field fluctuations, manufacturer TEE-GFAD logs)
        Oracle-->>Blockchain: 21. Provide Signed External Data (Post-Quantum Signature, Ethical Attestation)
    end
```
...
stateDiagram-v2
    direction LR
    state "CREATED (Pre-cognitive Intent, Ethical Blueprint)" as S0
    state "DIGITAL_TWIN_REGISTERED (Multiversal DTID, Ethical Lineage Validated)" as S1
    state "FUNDS_ESCROWED (Quantum-Resistant Multi-Currency, AI-Monitored for Equity)" as S2
    state "MANUFACTURER_AUTHORIZED (TEE-GFAD Certified, Ethical Audit & Predictive Compliance)" as S3
    state "PRODUCTION_INITIATED (Quantum Attested, Ethical Sourcing Confirmed)" as S4
    state "UPI_LINKED_ON_CHAIN (Quantum-Entangled, Molecular Imprint Verified)" as S5
    state "QUALITY_CONTROL_PASSED (Bio-Haptic AQC Verified, Sub-atomic Integrity)" as S6
    state "PACKAGING_COMPLETE (Ethical & Eco-Certified, Circular Economy Optimized)" as S7
    state "LOGISTICS_HANDOVER (Spatiotemporal Tracked, Ethical Logistics Verified)" as S8
    state "IN_TRANSIT (IoT & Atmospheric Analytics, Quantum Field Monitoring, Predictive Risk Mitigation)" as S9
    state "DELIVERED_CONFIRMED (Quantum Oracle Verified, Bio-Haptic User Feedback)" as S10
    state "FINAL_PAYMENT_RELEASED (AI-Validated Ethical & Performance Metrics)" as S11
    state "USER_VERIFIED (AR-PAD Confirmed, Quantum Coherence Authenticated)" as S12
    state "DISPUTE_INITIATED (AI-Driven Ethical Tribunal, Homeostasis Engine Triggered)" as S13
    state "DISPUTE_RESOLVED (Probabilistically Binding, System Self-Corrected)" as S14
    state "COMPLETED (Omniverse Integrity Confirmed, Ethical Homeostasis Achieved)" as S15
    state "CANCELED (Proactive & Ethical Withdrawal, AI-Recommended)" as S16
    state "REFUNDED (AI-Calculated & Instantaneous, Ethically Proportional)" as S17

    [*] --> S0: createOrder(PredictiveIntent, EthicalBlueprint)
    S0 --> S1: registerDigitalTwin(DTID_Multiversal, Ethical_AI_Provenance)
    S1 --> S2: escrowFunds(amount_MultiCurrency, AI_Equity_Check)
    S2 --> S3: verifyManufacturerVC(DID_SelfEvolving, Ethical_Compliance_Score)
    S3 --> S4: manufacturerAttest(ProdStarted_Quantum, EthicalSourcingConfirmed)
    S4 --> S5: manufacturerAttest(UPILinked_QuantumEntangled, MolecularImprint)
    S5 --> S6: quantumOracleAttest(AQC_Passed_Predictive, SubAtomicIntegrity)
    S6 --> S7: packagingComplete(EcoCertified, CircularEconomyOptimized)
    S7 --> S8: logisticsAttest(Handover_Spatiotemporal, EthicalLogisticsVC)
    S8 --> S9: logisticsAttest(TransitUpdate_Atmospheric_Quantum, PredictiveRisk)
    S9 --> S10: quantumOracleAttest(DeliveryConfirmed_Realtime, UserBioHapticFeedback)
    S10 --> S11: releaseFinalPayment(AI_Validated_Ethical_Performance)
    S11 --> S12: userVerify(UPI_ARPAD, QuantumChallenge)
    S12 --> S15: completeOrder(OmniverseConfirmed, EthicalHomeostasisMaintained)

    S0 --> S16: cancelOrder(Proactive, AI_Recommended_Ethical)
    S1 --> S16: cancelOrder(Proactive, AI_Recommended_Ethical)
    S2 --> S16: cancelOrder(Proactive, AI_Recommended_Ethical)
    S3 --> S16: cancelOrder(Proactive, AI_Recommended_Ethical)
    S4 --> S16: cancelOrder(Proactive, AI_Recommended_Ethical)
    S5 --> S16: cancelOrder(Proactive, AI_Recommended_Ethical)

    S11 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S10 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S9 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S8 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S7 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S6 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S5 --> S13: userDispute(reason_Ethical_or_Anomaly)
    S4 --> S13: userDispute(reason_Ethical_or_Anomaly)

    S13 --> S14: resolveDispute(AI_Tribunal_Ethical_Outcome, System_SelfCorrection)
    S14 --> S15: disputeSettled(OmniverseConfirmed, HomeostasisRestored)
    S14 --> S17: disputeRefund(AI_Calculated, EthicallyProportional)
    S17 --> [*]
    S15 --> [*]
    S16 --> [*]
```
...
```mermaid
sequenceDiagram
    participant Orchestrator as Orchestration Service (O'Callaghan Quantum Core & Homeostasis Engine)
    participant ManufInt as Manufacturing Integration Module (Quantum-Encrypted, Lattice-Based)
    participant ManufHSM as Card Manufacturing Facility (TEE-GFAD & Quantum-Secure, Gravitational Field Monitored)
    participant ManufProd as Manufacturing Production Line (Sub-atomic Precision, Quantum Optics)
    participant BlockSvc as Blockchain Interaction Service (Quantum-Hardened, Self-Healing)
    participant Blockchain as Fractal-Lattice Blockchain Layer (Quantum-Resilient)
    participant Oracle as Quantum Oracle Service (Pre-Cognitive, Ethical Data Sourcing)

    Orchestrator->>ManufInt: 1. Send Encrypted Production-Ready API & DTID (Lattice-Based Encryption, Ethical Provenance Hash)
    ManufInt->>ManufHSM: 2. Authenticate Facility DID & Decrypt API (Self-Evolving VC, Quantum Key via QKD, Ethical Compliance Check)
    ManufHSM->>ManufProd: 3. Securely Load Design Data to Production Line (Gravitational Field Monitored, Quantum Error Correction Applied)
    ManufHSM->>ManufHSM: 4. Generate Quantum-Entangled Unique Physical Identifier (UPI, Molecular Imprint Pattern)
    ManufHSM->>ManufProd: 5. Command Quantum-Entangled UPI Embedding (Atomic-Level Control)
    ManufProd->>ManufProd: 6. Produce Physical Card & Embed UPI (Atomic-Level Precision, Quantum Coherence Optimized)
    ManufProd->>ManufHSM: 7. Report Production Completion & UPI Value (Quantum-Coherence Confirmed, Molecular Signature Verified)
    ManufHSM->>ManufHSM: 8. Cryptographically Sign Attestation: {DTID, UPI, Production_Timestamp (Quantum-Sync), Manufacturer_DID, TEE-GFAD_Public_Key, Gravitational_Anomaly_Report_Hash, Quantum_Coherence_Check_Result, Ethical_Sourcing_Digest} (Post-Quantum Signature)
    ManufHSM->>ManufInt: 9. Send Signed Attestation (Quantum-Secured Channel, Lattice-Encrypted)
    ManufInt->>BlockSvc: 10. Forward Signed Attestation (with ZKP for Confidentiality)
    BlockSvc->>Blockchain: 11. Broadcast Transaction: link_UPI_Quantum(DTID, UPI, Attestation_Sig, Ethical_Digest)
    Blockchain-->>BlockSvc: 12. Transaction Confirmed (Fractal-Lattice Consensus, Quantum-Resilient Validation)
    BlockSvc-->>ManufInt: 13. Confirmation of UPI Linkage
    ManufInt-->>Orchestrator: 14. Confirmation of Production & UPI Linkage (with Predictive Integrity Score)
    ManufProd->>ManufHSM: 15. AQC System Report Data (Bio-Haptic, Hyperspectral, Sub-atomic Material Analysis)
    ManufHSM->>Oracle: 16. Attest AQC Results (Predictive Anomaly Detection, Ethical Compliance Check)
    Oracle->>BlockSvc: 17. Send Signed AQC Data (Post-Quantum Signature, Ethical Verification)
    BlockSvc->>Blockchain: 18. Broadcast Transaction: update_SC(AQC_Passed_Milestone_AI_Validated, Ethical_Compliance_Score)
```

**Figure 8: Multiversal Decentralized Identity (DID) and Self-Evolving Verifiable Credential (VC) Lifecycle**
```mermaid
sequenceDiagram
    participant User as User Wallet/App (Multi-Dimensional DID, Bio-Haptic, Quantum-Keyed)
    participant Issuer as Credential Issuer (AI-Driven Ethical, Regulatory DAO, Perpetual Audit)
    participant DIDReg as DID Registry (Fractal-Lattice Blockchain, Quantum-Resilient)
    participant Verifier as Verifier (Manufacturer, Logistics, Hyper-Intelligent Smart Contract, AI-Driven)

    User->>User: 1. Generate DID & DID Key Pair (Self-Sovereign, Quantum-Derived, Multi-Persona Capable)
    User->>DIDReg: 2. Register DID Document (Public Key, Service Endpoints, Quantum Attestations, Ethical Profile)
    DIDReg-->>User: 3. DID Registration Confirmation (Multiversal, Quantum-Sealed)

    User->>Issuer: 4. Request Verifiable Credential (e.g., "Certified User, Ethical Persona, Future-Compliant Citizen")
    Issuer->>Issuer: 5. Verify User's Identity (Off-chain KYC/AML, Bio-metric, Ethical AI Assessment, Recursive Ethical Provenance Chain)
    Issuer->>Issuer: 6. Create Self-Evolving Verifiable Credential (VC) signed by Issuer's DID (Post-Quantum Signature, Ethical Policy Hash)
    Issuer-->>User: 7. Issue VC to User (Stored in User's Quantum Wallet, with Temporal Evolution Rules)

    Verifier->>User: 8. Request Presentation of Specific VC (e.g., "Authorized Manufacturer, Future-Compliant, Ethical Sourcing Certified")
    User->>User: 9. Create Verifiable Presentation (VP) of VC (with ZKP for Post-Quantum Privacy & Entanglement Verifiability)
    User->>Verifier: 10. Present VP (via ZKP with Post-Quantum Entanglement Verifiability, Quantum-Secure Channel)
    Verifier->>DIDReg: 11. Resolve Issuer's DID to retrieve Public Key (Quantum-Resistant, Ethical Anchor)
    DIDReg-->>Verifier: 12. Return Issuer's DID Document (with Ethical Policy Hash)
    Verifier->>Verifier: 13. Verify Issuer's Signature on VC, User's Signature on VP, and VC Schema (Adaptive & Ethically Aligned)
    Verifier->>Verifier: 14. Access Granted / Transaction Authorized (Pre-Cognitive Confirmation, Ethical Score Incorporated)
    alt If VC is revoked (or predicted to be)
        Verifier->>DIDReg: 15. Check Revocation Status (e.g., Quantum Revocation List, Merkle Tree Root of Blacklisted Quantum States, Predictive Revocation Models)
        DIDReg-->>Verifier: 16. Return Revocation Status (Real-time & Predictive, Ethical Justification)
        Verifier->>Verifier: 17. Access Denied (with Ethical Justification, Perpetual Homeostasis Engine Notified)
    end
```
