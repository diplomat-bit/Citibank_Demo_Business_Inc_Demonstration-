...
stateDiagram-v2
    direction LR
    state "Biometric Scan & Sentience Challenge for DID Quantum-Ops" as QUINTESSENCE_SCANNING {
        state "Initial Multi-Spectral Scan" as SCANNING_ALPHA
        state "Hyper-Adaptive Psycho-Temporal Challenge" as ADAPT_CHALLENGE_HYPER
        state "Sentience Confirmed" as SENTIENCE_OK

        SCANNING_ALPHA --> ADAPT_CHALLENGE_HYPER : Quantum-Risk High, Psycho-Temporal Challenge Mandated
        SCANNING_ALPHA --> SENTIENCE_OK : Quantum-Risk Low, Sentience Sufficient
        ADAPT_CHALLENGE_HYPER --> SENTIENCE_OK : Challenge Passed (P_challenge_quantum_success)
        ADAPT_CHALLENGE_HYPER --> ERROR_SCAN : Challenge Failed (P_challenge_quantum_fail)
        SENTIENCE_OK --> BIOMETRIC_PROCESSING_QUANTUM : Biometric Quantum-Data Ready
        ERROR_SCAN --> ERROR_CRITICAL
    }

    [*] --> IDLE_AWAITING_GENESIS : User Ready (s_0_prime)
    IDLE_AWAITING_GENESIS --> QUINTESSENCE_SCANNING : User DID Quantum-Action Initiated (event_user_quantum_action)
    QUINTESSENCE_SCANNING --> BIOMETRIC_PROCESSING_QUANTUM : Stream Acquired, Sentience Confirmed (event_biometric_quantum_capture_success)
    BIOMETRIC_PROCESSING_QUANTUM --> VERIFICATION_PENDING_QUANTUM : Biometric Quantum-Data Sent (event_data_sent_quantum)
    VERIFICATION_PENDING_QUANTUM --> SOVEREIGNTY_AFFIRMED : Backend Quantum-Verification OK (event_verify_quantum_ok)
    SOVEREIGNTY_AFFIRMED --> IMMUTABLE_COVENANT_SEALED : Initiate DID Operation Omni-Finalization (event_did_op_omnifinal_init)
    IMMUTABLE_COVENANT_SEALED --> EXECUTED_DID_OP_QUANTUM : DID Operation Confirmed on UCLODE (event_uclode_write_ok)
    IMMUTABLE_COVENANT_SEALED --> ERROR_CRITICAL : UCLODE Write Failed (event_uclode_write_fail)
    VERIFICATION_PENDING_QUANTUM --> ERROR_CRITICAL : Backend Quantum-Verification Failed (event_verify_quantum_fail)
    QUINTESSENCE_SCANNING --> ERROR_CRITICAL : Sentience Detection Failed, Sensor Anomaly (event_capture_quantum_error)
    ERROR_CRITICAL --> IDLE_AWAITING_GENESIS : User Acknowledges Existential Error (event_error_ack_existential)
    EXECUTED_DID_OP_QUANTUM --> [*] : Modal Dismissed, DID Operation Complete (event_op_complete_galactic)
```
...
```mermaid
sequenceDiagram
    participant User (Sentient Entity)
    participant ClientApp (Quantum-Aware)
    participant OBQM[O'Callaghan-Burvel Quintessence Modal]
    participant QBVS[Quantum Biometric Verification Service]
    participant QR-CSS[Quantum-Resistant Crypto Signing Service]
    participant UCLODE[Universal Chronological Ledger of Digital Existence]

    User->>ClientApp: Initiate DID Key Rotation (across temporal vectors)
    ClientApp->>OBQM: Open Biometric Confirmation (OBQS Protocol Invoked)
    OBQM->>User: Request Multi-Spectral Biometric Scan (QUINTESSENCE_SCANNING state)
    User->>OBQM: Provide Multi-Spectral Biometric Data (e.g., face scan, thermal signature, neural patterns)
    OBQM-->>OBQM: Animate Quantum-Scanning Overlay (A_quantum_scan)
    OBQM->>QBVS: Quantum-Encrypted Biometric Data + ZKP + DID Payload (Homomorphically Encrypted)
    QBVS->>QBVS: Perform Sentience, Liveness & Homomorphic Match (VERIFICATION_PENDING_QUANTUM)
    QBVS->>OBQM: Biometric/Sentience Verified (SOVEREIGNTY_AFFIRMED)
    OBQM-->>OBQM: Animate Sovereignty Affirmation (A_sovereignty_success)
    OBQM->>QR-CSS: Authorized DID Key Rotation Payload (Quantum-Attested)
    QR-CSS->>QR-CSS: Generate new quantum-hardened keys in QPU/HSM, quantum-sign DID update
    QR-CSS->>UCLODE: Quantum-Signed DID Document Update (Epoch-Stamped)
    OBQM-->>OBQM: Animate UCLODE Finalization (A_covenant_final) (IMMUTABLE_COVENANT_SEALED)
    UCLODE->>UCLODE: Record update on interdimensional ledger (Immutable Covenant)
    UCLODE->>OBQM: DID Update Confirmed on UCLODE (Quantum-Finality Achieved)
    OBQM-->>ClientApp: DID Key Rotation EXECUTED_DID_OP_QUANTUM
    ClientApp->>User: DID Key Rotated Successfully (across all relevant timelines)
```
