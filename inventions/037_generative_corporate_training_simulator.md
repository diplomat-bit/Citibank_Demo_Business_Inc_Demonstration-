...
export class TrainingSessionManager {
    ...
    /**
     * Ends the current training session and potentially returns a summary.
     * @returns {Promise<any>} A summary report or analysis of the session.
     */
    public async endSession(): Promise<any> {
        ...
        ----------------------------------
        `;
        const finalReport = await this.coachChatAgent.sendMessage({ message: finalSummaryPrompt });
        return {
...
*   **Definition 1.1: Conversational State Space ($\mathcal{S}$)**
    Let $\mathcal{S}$ be a continuous state space representing the comprehensive contextual and emotional landscape of the simulated interaction. An element $s_t \in \mathcal{S}$ at time $t$ is a vector incorporating:
    *   $\mathbf{s}_{t, \text{persona}} \in \mathbb{R}^{d_P}$: A vector representing the Persona Emulation Module's internal state (e.g., frustration level, compliance index, conversational objectives, memory of past turns).
    *   $\mathbf{s}_{t, \text{scenario}} \in \mathbb{R}^{d_S}$: A vector representing global scenario parameters (e.g., time pressure, stakes, specific facts of the case).
    *   $\mathbf{s}_{t, \text{linguistic}} \in \mathbb{R}^{d_L}$: A latent vector representing the cumulative semantic and pragmatic interpretation of the preceding dialogue history.
    Thus, $s_t = [\mathbf{s}_{t, \text{persona}}, \mathbf{s}_{t, \text{scenario}}, \mathbf{s}_{t, \text{linguistic}}]$.

*   **Definition 1.2: User Utterance Space ($\mathcal{U}$)**
    Let $\mathcal{U}$ be the space of all possible linguistic inputs from the user. An utterance $u_t \in \mathcal{U}$ at time $t$ is a sequence of tokens, which can be represented in a high-dimensional embedding space, $\mathbf{u}_t \in \mathbb{R}^{d_U}$.

*   **Definition 1.3: Persona Response Space ($\mathcal{P}$)**
    Let $\mathcal{P}$ be the space of all possible linguistic outputs from the Persona Emulation Module. A persona response $p_t \in \mathcal{P}$ at time $t$ is also a sequence of tokens, $\mathbf{p}_t \in \mathbb{R}^{d_P'}$.
...
*   **Definition 2.1: User Conversational Policy ($\Pi_H(\cdot|s_t; \theta)$)**
    The user's communicative policy is a conditional probability distribution $\Pi_H: \mathcal{S} \times \mathcal{U} \rightarrow [0,1]$, representing the probability of the user emitting utterance $u_t$ given the current state $s_t$ and a set of internal skill parameters $\theta$.
    $\Pi_H(u_t | s_t; \theta) = P(U_t = u_t | S_t = s_t, \text{UserParams} = \theta)$.
    The parameter vector $\theta \in \mathbb{R}^k$ encapsulates the user's proficiency across various communication skills (e.g., $\theta_1$ for empathy, $\theta_2$ for de-escalation, $\theta_3$ for clarity, etc.). These parameters are not directly observed but are implicitly updated by the user.

*   **Definition 2.2: Persona State Transition Function ($\mathcal{T}_P$)**
    ...
    $\mathcal{T}_P: \mathcal{S} \times \mathcal{U} \rightarrow \mathcal{S} \times \mathcal{P}$.
    Formally, $P(s_{t+1}, p_t | s_t, u_t) = \mathcal{N}(f_P(s_t, u_t), \Sigma_P)$, where $f_P$ is a complex, non-linear function implemented by the generative LLM, and $\mathcal{N}$ represents inherent stochasticity.
...
*   **Definition 3.1: Pedagogical Feedback Module (PFM) Function ($\Phi_C$)**
    ...
    $\Phi_C: \mathcal{S} \times \mathcal{U} \rightarrow \mathbb{R}^m$.
    Here, $m$ is the number of distinct evaluation criteria (e.g., $r_{EV}$, $r_{PIC}$, $r_{SCC}$, $r_{PDR}$), where each $r_j \in [0, 10]$ is a scalar score for a specific competency.
    The output is a vector $\mathbf{R}_t = [r_{EV}, r_{PIC}, r_{SCC}, r_{PDR}, \dots]^T$.

*   **Definition 3.2: Expert Evaluation Oracle ($\Omega_{exp}$)**
    The function $\Phi_C$ is fundamentally an approximation of an ideal "Expert Evaluation Oracle," $\Omega_{exp}$, which perfectly assesses an utterance against all possible pedagogical rubrics. The PFM is trained and refined to minimize the divergence between its output and $\Omega_{exp}$. This involves sophisticated natural language understanding (NLU), rhetorical analysis, and adherence to formalized communication frameworks (e.g., ACS).
    Formally, $\Phi_C(s_t, u_t) = g_C(\text{embedding}(s_t), \text{embedding}(u_t); \psi)$, where $g_C$ is the function implemented by the PFM LLM, and $\psi$ represents its internal knowledge base and parameters.

*   **Definition 3.3: Pedagogical Utility Function ($\mathcal{J}$) over $\mathbf{R}_t$**
    ...
    $\mathcal{J}(\theta) = E_{s_t, u_t \sim \Pi_H(\cdot|s_t; \theta)} \left[ \sum_{t=0}^{T} \gamma^t \cdot \mathbf{w}^T \mathbf{R}_t \right]$
    where $\mathbf{w} \in \mathbb{R}^m$ is a vector of pedagogical weights, potentially user-configurable, indicating the relative importance of each skill dimension.
...
*   **Theorem 4.1 (Implicit Policy Gradient Theorem):**
    Given the objective function $\mathcal{